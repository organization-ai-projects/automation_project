...
Each project includes a `metadata.ron` file that describes its capabilities and entry points. This file is used by the global registry and the central UI to discover and display projects. See [Registre](registry.md) for registry structure and ID format.
