...
To simplify branch creation, you can use the `create_branch.sh` script:

```bash
./scripts/versioning/file_versioning/git/create_branch.sh <branch-name>
```

This script creates a new branch from `dev`, updates it, and sets up remote tracking.

See the script documentation for details: [Script: create_branch.sh](scripts/create_branch.md).
