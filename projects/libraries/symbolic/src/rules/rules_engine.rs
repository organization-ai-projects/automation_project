...
mod tests {
    ...
    fn test_extract_name() {
        ...
        assert_eq!(
            engine.extract_name("create struct User"),
            Some("User".to_string())
        );

        assert_eq!(
            engine.extract_name("define enum Status"),
    ...
    fn test_extract_fields() {
        ...
        let fields = engine.extract_fields("struct User with name and email", None);
        assert!(fields.is_some());

        let fields_str = fields.unwrap();
        assert!(fields_str.contains("name"));
        assert!(fields_str.contains("email"));
    }
