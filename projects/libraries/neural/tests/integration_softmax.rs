...
fn test_integration_softmax() {
    ...
    assert!(probs[2] > probs[1], "logit 3.0 should produce higher probability than 2.0");
    assert!(probs[1] > probs[0], "logit 2.0 should produce higher probability than 1.0");
    
    // All probabilities should be non-negative and <= 1.0
    for (i, &prob) in probs.iter().enumerate() {
        assert!(prob >= 0.0 && prob <= 1.0, "probability at index {} should be in [0, 1], got {}", i, prob);
    }
}
