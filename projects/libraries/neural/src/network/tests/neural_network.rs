...
fn test_network_creation() {
    ...
    assert_eq!(network.input_size(), 4, "input size should match configuration");
    assert_eq!(network.output_size(), 1, "output size should match configuration");
}
...
fn test_forward_backward() {
    ...
    let output = network.forward(&input).expect("forward pass should succeed");
    assert_eq!(output.len(), 1, "output should have correct dimension");
    assert!(output[0].is_finite(), "output should be a finite number");

    let loss = network.backward(&target, 0.01).expect("backward pass should succeed");
