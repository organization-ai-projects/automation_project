...
where
    ...
    F: FnOnce(&AstNode) -> T,
{
    let value = node.get(key).unwrap_or_else(|| panic!("Missing key '{}'", key));
...
pub(crate) fn assert_error_kind(result: Result<(), AstValidationError>, expected_kind: fn(&AstErrorKind) -> bool) {
    assert!(result.is_err(), "Expected an error but got Ok");
    let err = result.unwrap_err();
    assert!(expected_kind(&err.kind), "Error kind mismatch: {:?}", err.kind);
}
...
pub(crate) fn assert_string_key(node: &AstNode, key: &str, expected: &str) {
    let value = node.get(key).unwrap_or_else(|| panic!("Missing key '{}'", key));
    assert_eq!(value.as_string(), Some(expected), "Key '{}' has unexpected value", key);
}
...
pub(crate) fn assert_bool_key(node: &AstNode, key: &str, expected: bool) {
    let value = node.get(key).unwrap_or_else(|| panic!("Missing key '{}'", key));
    assert_eq!(value.as_bool(), Some(expected), "Key '{}' has unexpected value", key);
}
...
pub(crate) fn assert_nested_string(node: &AstNode, keys: &[&str], expected: &str) {
    ...
    for (i, key) in keys.iter().enumerate() {
        current = current.get(key).unwrap_or_else(|| {
            panic!("Missing key '{}' at depth {}", key, i)
        });
    }
    assert_eq!(current.as_string(), Some(expected), "Nested key path {:?} has unexpected value", keys);
}
