// projects/products/unstable/platform_ide/src/tests/contract_tests.rs
...
fn manifest_rejects_traversal_attacks() {
    ...
    // Even if somehow "../etc/passwd" was in the manifest, path validation
...
fn verification_forbidden_path_error_becomes_generic_without_path() {
    ...
    assert_eq!(view.findings.len(), 1);
    let f = &view.findings[0];
    // The forbidden path must not appear in any output field.
...
fn verification_forbidden_path_info_finding_is_dropped() {
    ...
    // Info-level findings for forbidden paths must be silently dropped.
    assert!(
