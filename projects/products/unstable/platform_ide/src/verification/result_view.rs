// projects/products/unstable/platform_ide/src/verification/result_view.rs
...
impl VerificationResultView {
    ...
    ///
    /// For forbidden-area findings the finding is preserved with a generic
    /// message and no path/line details, consistent with the contract-level
    /// error policy.
    pub fn from_raw(
        healthy: bool,
    ...
    fn filter_finding(raw: RawFinding, manifest: &SliceManifest) -> Option<VerificationFinding> {
        match raw.path.as_deref() {
            None => {
                // No path associated — always safe to show.
            ...
            Some(p) => {
                if manifest.allow(p).is_ok() {
                    // Path is in the slice — show full details.
                    Some(VerificationFinding {
                        severity: raw.severity,
                        summary: raw.summary,
                    ...
                    // Forbidden path — emit a contract-level error with no details.
                    // Only emit for errors/warnings; suppress info findings entirely.
                    match raw.severity {
...
mod tests {
    ...
    fn forbidden_path_error_finding_becomes_generic() {
        ...
        // The original summary must not be revealed.
    ...
    fn forbidden_path_info_finding_is_suppressed() {
        ...
        // Info findings for forbidden paths are dropped entirely.
