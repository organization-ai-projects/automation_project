...
use std::process;
use auto_manager_ai::{generate_action_plan, evaluate_plan, write_outputs, Config, RunReport};
...
fn main() {
    ...
    let plan = match generate_action_plan(&config) {
        ...
        Err(e) => {
            ...
            report.add_error(e);
            if let Err(write_err) = write_outputs(&auto_manager_ai::ActionPlan::new("Error".to_string()), &report, &config.output_dir) {
