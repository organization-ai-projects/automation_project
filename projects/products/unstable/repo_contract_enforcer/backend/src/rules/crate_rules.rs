//! projects/products/unstable/repo_contract_enforcer/backend/src/rules/crate_rules.rs
    ...
    pub fn evaluate(
        ...
        for (crate_name, crate_path, expected_name) in [
            ...
            if cargo.exists() {
                ...
                if txt.contains("[[bin]]") {
                    let cli_name = read_cli_name_from_metadata(crate_path);
                    ...
                    if !allowed {
                        out.push(make_violation(
                            ...
                            (scope, mode),
                            &cargo,
                            "crate must be bin-only (no [[bin]]); to declare a CLI name, set cli_name in metadata.ron and use [[bin]] name = cli_name",
                            (true, None),
                        ));
                    }
                }
...
fn is_file_under_tests_dir(src_dir: &path::Path, rs_file: &path::Path) -> bool {
    let rel = match rs_file.strip_prefix(src_dir) {
        Ok(path) => path,
        Err(_) => return false,
    };
...
fn allowed_inline_test_attr_lines_for_mod_rs(content: &str) -> collections::HashSet<u32> {
    let mut allowed = collections::HashSet::new();
...
fn expected_paired_test_path(rs_file: &path::Path) -> Option<path::PathBuf> {
    let stem = rs_file
        .file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or_default();
...
/// Reads `cli_name` from `metadata.ron` located at the crate root.
/// Returns `None` if the file is absent, unreadable, or `cli_name` is not set.
fn read_cli_name_from_metadata(crate_path: &path::Path) -> Option<String> {
    let metadata_path = crate_path.join("metadata.ron");
...
fn parse_bin_name_from_cargo(cargo_toml: &str) -> Option<String> {
    ...
    for line in cargo_toml.lines() {
        ...
        // New section starts — exit bin section
