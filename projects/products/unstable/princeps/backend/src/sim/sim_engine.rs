...
impl SimEngine {
    ...
    fn generate_report(&self, days: u32) -> Result<EndReport, PrincepsError> {
        ...
        let canonical = canonical_report_payload(&self.game_id, &winner, &final_poll, &run_summary);
...
pub(crate) fn canonical_report_payload(
    game_id: &GameId,
    winner: &CandidateId,
    final_poll: &PollReport,
    run_summary: &RunSummary,
) -> String {
    let mut out = String::new();
    out.push_str("game_id=");
    out.push_str(&game_id.to_string());
    out.push_str("|winner=");
    out.push_str(&winner.to_string());
    out.push_str("|final_poll.day=");
    out.push_str(&final_poll.day.to_string());

    out.push_str("|final_poll.results=");
    for (candidate_id, score) in &final_poll.results {
        out.push_str(&candidate_id.to_string());
        out.push(':');
        out.push_str(&format!("{score:.12}"));
        out.push(',');
    }

    out.push_str("|final_poll.block_breakdown=");
    for (block_id, scores) in &final_poll.block_breakdown {
    ...
    out.push_str("|run_summary.seed=");
    out.push_str(&run_summary.seed.to_string());
    out.push_str("|run_summary.days=");
    out.push_str(&run_summary.days.to_string());
    out.push_str("|run_summary.total_events=");
    out.push_str(&run_summary.total_events.to_string());
    out.push_str("|run_summary.total_debates=");
    out.push_str(&run_summary.total_debates.to_string());
    out.push_str("|run_summary.total_polls=");
    out.push_str(&run_summary.total_polls.to_string());

    out.push_str("|run_summary.candidate_final_approvals=");
    for (candidate_id, approval) in &run_summary.candidate_final_approvals {
        out.push_str(&candidate_id.to_string());
        out.push(':');
        out.push_str(&format!("{approval:.12}"));
        out.push(',');
    }
...
fn apply_campaign_event(event: &CampaignEvent, candidates: &mut [Candidate]) {
