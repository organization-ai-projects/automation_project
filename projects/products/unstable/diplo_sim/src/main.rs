...
fn print_usage() {
    eprintln!("Usage:");
    eprintln!("  diplo_sim run --turns N --seed S --map <map_file> --players <n> --out <match.json> [--replay-out <replay.bin>]");
    ...
    let result = match args[1].as_str() {
        "run" => {
            let turns: u32 = get_arg(&args, "--turns")
                ...
                .unwrap_or_else(|| { eprintln!("Missing --turns"); std::process::exit(2); });
            let seed: u64 = get_arg(&args, "--seed")
                .and_then(|v| v.parse().ok())
                .unwrap_or_else(|| { eprintln!("Missing --seed"); std::process::exit(2); });
            let map_path = get_arg(&args, "--map")
                .unwrap_or_else(|| { eprintln!("Missing --map"); std::process::exit(2); });
            let players: u32 = get_arg(&args, "--players")
                .and_then(|v| v.parse().ok())
                .unwrap_or_else(|| { eprintln!("Missing --players"); std::process::exit(2); });
            let out = get_arg(&args, "--out")
                .unwrap_or_else(|| { eprintln!("Missing --out"); std::process::exit(2); });
            let replay_out = get_arg(&args, "--replay-out");

            config::runner::run_simulation(turns, seed, &map_path, players, &out, replay_out.as_deref())
        }
        "replay" => {
            let replay_path = get_arg(&args, "--replay")
                .unwrap_or_else(|| { eprintln!("Missing --replay"); std::process::exit(2); });
            let out = get_arg(&args, "--out")
                .unwrap_or_else(|| { eprintln!("Missing --out"); std::process::exit(2); });
        ...
        "validate-orders" => {
            let map_path = get_arg(&args, "--map")
                .unwrap_or_else(|| { eprintln!("Missing --map"); std::process::exit(2); });
            let orders_path = get_arg(&args, "--orders")
                .unwrap_or_else(|| { eprintln!("Missing --orders"); std::process::exit(2); });
