...
fn make_triangle_map() -> MapGraph {
    diplo_sim::map::map_loader::load_map_from_str(
...
fn test_adjudication_deterministic_same_orders() {
    ...
    let order_sets = vec![
        ...
        OrderSet::new(
            FactionId(1),
            vec![Order { id: OrderId(1), unit_id: UnitId(1), kind: OrderKind::Hold }],
        ),
    ];
    ...
    assert_eq!(report1, report2, "Same orders should produce identical reports");
}
...
fn test_tie_breaking_lower_unit_id_wins() {
    ...
    let order_sets = vec![
        OrderSet::new(
            ...
            vec![Order {
                id: OrderId(0),
                unit_id: UnitId(0),
                kind: OrderKind::Move { target: TerritoryId(2) },
            }],
        ),
        OrderSet::new(
            FactionId(1),
    ...
    let step0 = report.steps.iter().find(|s| s.unit_id == UnitId(0)).unwrap();
    let step1 = report.steps.iter().find(|s| s.unit_id == UnitId(1)).unwrap();
...
fn test_move_succeeds_against_empty_territory() {
    ...
    let state = GameState {
        ...
        factions: vec![Faction { id: FactionId(0), name: "F0".into() }],
        current_turn: Turn::new(0),
        map_graph: map,
    };
    ...
    let step = report.steps.iter().find(|s| s.unit_id == UnitId(0)).unwrap();
