...
fn test_valid_move_to_adjacent_territory_passes() {
    ...
    assert!(
        errors.is_empty(),
        "Move to adjacent territory should be valid: {:?}",
        errors
    );
}
...
fn test_move_to_non_adjacent_territory_is_rejected() {
    ...
    assert!(
        !errors.is_empty(),
        "Move to non-adjacent territory should be rejected"
    );
    ...
    assert!(
        ...
        "Error should reference the correct order_id, unit_id, and territory_id. Got: {:?}",
        first_err
    );
}
...
fn test_invalid_unit_id_is_rejected() {
    ...
    assert!(
        !errors.is_empty(),
        "Order with invalid unit_id should be rejected"
    );
    let first = &errors[0];
    assert!(
        matches!(first, DiploSimError::OrderValidation { unit_id, .. } if *unit_id == UnitId(999))
    );
}
...
fn test_order_for_wrong_faction_is_rejected() {
    ...
    assert!(
        !errors.is_empty(),
        "Ordering a unit from wrong faction should be rejected"
    );
}
