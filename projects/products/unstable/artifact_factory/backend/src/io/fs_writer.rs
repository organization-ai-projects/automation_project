...
impl FsWriter {
    ...
    pub fn write_bundle(bundle: &ArtifactBundle, output_dir: &Path) -> Result<(), BackendError> {
        std::fs::create_dir_all(output_dir).map_err(|e| BackendError::Io(e.to_string()))?;
        for name in &bundle.manifest {
            if let Some(bytes) = bundle.files.get(name) {
                let file_path = output_dir.join(name);
                std::fs::write(&file_path, bytes).map_err(|e| BackendError::Io(e.to_string()))?;
            }
        }
        Ok(())
    }
