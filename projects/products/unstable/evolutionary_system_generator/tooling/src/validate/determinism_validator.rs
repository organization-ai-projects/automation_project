...
impl DeterminismValidator {
    pub fn validate(config: ValidatorConfig, backend_bin: &str) -> Result<DeterminismResult, ToolingError> {
        let hash1 = Self::run_once(&config, backend_bin)?;
        let hash2 = Self::run_once(&config, backend_bin)?;
        let ok = hash1 == hash2;
        Ok(DeterminismResult { determinism_ok: ok, manifest_hash: Some(hash1) })
    }
