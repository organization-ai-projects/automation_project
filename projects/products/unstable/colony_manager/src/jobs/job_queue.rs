...
impl JobQueue {
    pub fn add(&mut self, job: Job) { self.jobs.insert(job.id, job); }
    pub fn remove(&mut self, id: &JobId) -> Option<Job> { self.jobs.remove(id) }
    pub fn unassigned_sorted(&self) -> Vec<&Job> {
        let mut jobs: Vec<&Job> = self.jobs.values()
