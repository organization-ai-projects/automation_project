...
impl EventDeck {
    pub fn default_deck() -> Self {
        Self {
            events: vec![
                ...
                ColonyEvent::Sickness {
                    colonist_name: "unknown".to_string(),
                },
                ColonyEvent::Traders {
                    goods: vec!["food".to_string(), "tools".to_string()],
                },
                ColonyEvent::Windfall {
    ...
    pub fn draw(
        ...
        draw_log: &mut Vec<RngDraw>,
    ) -> Option<(usize, &ColonyEvent)> {
        if self.events.is_empty() {
            return None;
        }
        let raw = rng.next_u64();
        let idx = (raw % self.events.len() as u64) as usize;
        draw_log.push(RngDraw {
            raw_value: raw,
            resolved_index: idx,
        });
