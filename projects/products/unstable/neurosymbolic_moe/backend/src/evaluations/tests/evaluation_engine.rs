//! projects/products/unstable/neurosymbolic_moe/backend/src/evaluations/tests/evaluation_engine.rs
...
fn governance_report_flags_underperforming_experts() {
    let mut engine = EvaluationEngine::new();
    let expert_good = ExpertId::new();
    let expert_bad = ExpertId::new();
