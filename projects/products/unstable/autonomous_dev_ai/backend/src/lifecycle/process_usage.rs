//! projects/products/unstable/autonomous_dev_ai/backend/src/lifecycle/process_usage.rs
...
pub(crate) struct ProcessUsage {
    ...
    pub(crate) rss_bytes: usize,
}

impl ProcessUsage {
    pub(crate) fn sample() -> Option<Self> {
        Some(Self {
...
fn read_cpu_time_from_proc_stat() -> Option<Duration> {
    ...
    let ticks_per_second = env::var("AUTONOMOUS_CPU_TICKS_PER_SECOND")
        .ok()
