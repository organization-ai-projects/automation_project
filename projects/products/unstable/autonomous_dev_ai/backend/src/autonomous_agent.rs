...
use crate::persistence::{
    self, ActionOutcomeStats, LearningSnapshot, append_learning_snapshot,
    load_action_outcome_index, load_decision_inverted_index, load_failure_inverted_index,
    load_memory_state_index, load_memory_state_with_fallback, load_recent_learning_snapshots,
    memory_transaction_completed, save_memory_state_transactional,
};
use crate::value_types::{ActionName, ConfidenceScore, LearningWindow};

//Autonomous developer AI agent
pub struct AutonomousAgent<'a> {
    pub lifecycle: LifecycleManager<'a>,
    state_path: StatePath,
}

impl<'a> AutonomousAgent<'a> {
    ...
    pub fn load_state(&mut self) -> AgentResult<()> {
        ...
        if let Some(action_outcome_index) = load_action_outcome_index(&self.state_path)? {
            if let Some((action, stats)) = select_worst_action_outcome(&action_outcome_index) {
                let summary = ActionOutcomeStats {
                ...
                self.lifecycle.memory.metadata.insert(
                    ...
                    common_json::to_string(&summary).unwrap_or_else(|_| {
                        format!(
                            "{}:{:.3}:{}",
                            summary.action,
    ...
    pub fn run_symbolic_only(&mut self, goal: &str) -> AgentResult<()> {
        ...
        // Disable neural
        self.lifecycle.artifacts.neural.enabled = false;
...
fn select_worst_action_outcome(
    index: &persistence::ActionOutcomeIndex,
) -> Option<(String, persistence::ActionOutcomeStats)> {
    index
        .by_action
        .iter()
