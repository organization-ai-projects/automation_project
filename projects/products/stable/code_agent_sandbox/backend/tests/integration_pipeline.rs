...
#[test]
fn test_allowed_write_request() -> anyhow::Result<()> {
...
#[test]
fn test_forbidden_write_request() -> anyhow::Result<()> {
    let (_temp_dir, repo_root_s, runs_root_s) = setup_temp_dirs();
...
fn test_invalid_cli_args() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_code_agent_sandbox_backend"))
        .output()
