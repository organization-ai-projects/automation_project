...
use crate::{
    pr::commands::pr_field_name::PrFieldName, pr_remote_snapshot::load_pr_remote_snapshot,
    ...
    pub(crate) fn run_field(self) -> i32 {
        ...
        match self.name {
            PrFieldName::CommitMessages => {
                let out = load_pr_remote_snapshot(&self.pr_number, &repo_name)
            ...
            _ => {
                let snapshot =
                    load_pr_remote_snapshot(&self.pr_number, &repo_name).unwrap_or_default();
