...
use crate::pr::commands::{
    ...
    PrDirectiveConflictGuardOptions, PrDirectiveConflictsOptions, PrDirectivesApplyOptions,
    PrDirectivesFormat, PrDirectivesOptions, PrDirectivesStateOptions, PrDuplicateActionsOptions,
    PrEffectiveCategoryOptions, PrFieldName, PrFieldOptions, PrGenerateDescriptionOptions,
    PrGroupByCategoryOptions, PrIssueCategoryFromLabelsOptions, PrIssueCategoryFromTitleOptions,
    PrIssueClosePolicyOptions, PrIssueContextOptions, PrIssueDecisionOptions,
    PrIssueRefKindOptions, PrIssueViewOptions, PrNonClosingRefsOptions, PrNormalizeIssueKeyOptions,
    PrOpenReferencingIssueOptions, PrPrStateOptions, PrRefreshValidationOptions,
    PrResolveCategoryOptions, PrSortBulletsOptions, PrTextPayloadOptions, PrUpdateBodyOptions,
    PrUpsertCommentOptions,
};
...
fn parse_upsert_comment(args: &[String]) -> Result<PrUpsertCommentOptions, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            unknown => return Err(format!("Unknown option for upsert-comment: {unknown}")),
        }
    }

    require_positive_number("--pr", &pr_number)?;
    if marker.is_empty() {
        return Err("--marker is required".to_string());
    }
    if body.is_empty() {
        return Err("--body is required".to_string());
    }

    Ok(PrUpsertCommentOptions {
        pr_number,
        repo,
        marker,
        body,
...
fn parse_text_payload(args: &[String]) -> Result<PrTextPayloadOptions, String> {
    let mut pr_number = String::new();
    let mut repo: Option<String> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            unknown => return Err(format!("Unknown option for text-payload: {unknown}")),
        }
    }

    require_positive_number("--pr", &pr_number)?;
    Ok(PrTextPayloadOptions { pr_number, repo })
}
