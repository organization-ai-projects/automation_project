use crate::{
    automation::{current_branch, ensure_git_repo, execute::branch_exists_remote, run_git},
    git_cli,
};
...
impl CheckMergeConflictsOptions {
    pub(crate) fn run_check_merge_conflicts(self) -> Result<(), String> {
        ensure_git_repo()?;
        ...
        run_git(&["fetch", "--prune", &self.remote])?;
        if !branch_exists_remote(&self.remote, &self.base_branch) {
            return Err(format!(
                "Base branch '{}/{}' does not exist.",
                self.remote, self.base_branch
            ));
        }
